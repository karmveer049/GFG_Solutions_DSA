...
class Solution {
  public:
   ...
   return calCost(0,n,arr,k,trace);
    }
    int calCost(int curr, int n, vector<int>&arr, int k,vector<int>&trace){
        if(curr>=n) return  0;
        if(trace[curr]!=-1) return trace[curr];
        int ans=INT_MAX;
        
        int sum=0;
        for(int i=curr;i<n;i++){
            sum+=arr[i];
            int tspace=sum+(i-curr);
            if(tspace>k) break;
            if(i!=n-1){
                int temp=(k-tspace)*(k-tspace) + calCost(i+1,n,arr,k,trace);
            ans=min(ans,temp);
            }
            else ans=0;
        }
        return trace[curr]=ans;
    }
